...
def save_inputs_and_users(path: Path, inputs: pd.DataFrame, users: pd.DataFrame) -> None:
    save_sheet_replace(path, SHEET_INPUTS, inputs)
...
def main() -> None:
    ...
    with tab[0]:
        ...
        if submitted:
            ...
            try:
                save_sheet_replace(path, SHEET_INPUTS, inputs2)
            except Exception as exc:
                st.error(f"Failed to save your submission to Excel: {exc}")
            else:
                st.success("Saved! Your submission has been added to the Inputs sheet.")
